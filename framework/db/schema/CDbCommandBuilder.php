...
class CDbCommandBuilder extends CComponent {
    ...
    public function createUpdateCounterCommand($table, $counters, $criteria) {
        ...
        if ($fields !== array()) {
            ...
            throw new CDbException(Yii::t('yii', 'No counter columns are being updated for table "{table}".', array('{table}' => $table->name)));
    }
    ...
    public function createColumnCriteria($table, $columns, $condition = '', $params = array(), $prefix = null) {
        ...
        foreach ($columns as $name => $value) {
            if (($column = $table->getColumn($name)) !== null) {
                ...
                throw new CDbException(Yii::t('yii', 'Table "{table}" does not have a column named "{column}".', array('{table}' => $table->name, '{column}' => $name)));
        }
    ...
    public function createInCondition($table, $columnName, $values, $prefix = null) {
        ...
        if (is_string($columnName)) { // simple key
            if (!isset($table->columns[$columnName]))
                throw new CDbException(Yii::t('yii', 'Table "{table}" does not have a column named "{column}".', array('{table}' => $table->name, '{column}' => $columnName)));
            ...
            foreach ($columnName as $name) {
                ...
                for ($i = 0; $i < $n;  ++$i) {
                    if (isset($values[$i][$name])) {
                        ...
                        throw new CDbException(Yii::t('yii', 'The value for the column "{column}" is not supplied when querying the table "{table}".', array('{table}' => $table->name, '{column}' => $name)));
                }
            }
            if (count($values) === 1) {
                $entries = array();
